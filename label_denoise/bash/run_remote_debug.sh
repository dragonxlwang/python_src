...
#hadoop params   
mapper="$GPYTHON test.py --debug -m";
reducer="$GPYTHON test.py --debug -r";
#reducer="cat";
...
input_files_param_str=$(echo $trainDayLst | \
					   sed -r "s@\S+@-input ${nn}/user/wangxl/data/data_with_tag/&/finalpvs.out@g");
output_file="${nn}/user/wangxl/avg_fv.out";
