...
import topic_modeling.lda_gibbs_sampling;
import os.path;
...
def citationLdaRun(data, K, D, W, alpha, beta, burninTimeHr, sampliTimeHr, dumpFileFolder):
    ...
    ldaInstance = topic_modeling.lda_gibbs_sampling.LDA(data, K, D, W, alpha, beta, burnTime=burninTimeHr, sampTime=sampliTimeHr, iterCtrl=False);
    (postTheta, postPhi, topicWeights) = ldaInstance.Mcmc();
    topic_modeling.lda_gibbs_sampling.dumpLdaEstimateFile(ldaInstance, dumpFilePath)
...
def getTopicTimeDistribution(ldaInstance, pmd, pmidToId, idToPmid):
    begYr = 1950;
    endYr = 2012;
    topicTimeMatrix = [[0.0 for i in range(endYr - begYr + 1)] for k in range(ldaInstance.K)];
    phiMatrix = ldaInstance.phiEstimate;
    for pmid in pmd.docs:
        if(pmid not in pmidToId): continue;
        year = pmd.docs[pmid]['year'];
        if(not (year <= endYr and year >= begYr)): continue;
        for k in range(ldaInstance.K): 
            topicTimeMatrix[k][year - begYr] += phiMatrix[k][pmidToId[pmid]];
    return (begYr, endYr, topicTimeMatrix);

def getCitationPrediction(ldaInstance, data):
    thetaMatrix = ldaInstance.thetaEstimate;
    phiMatrix = ldaInstance.phiEstimate;
    correctCnt = 0.0;
    wrongCnt = 0.0;
    maxP = 0;
    for (d, w, c) in data:
        p = sum([thetaMatrix[d][k] * phiMatrix[k][c] for k in range(ldaInstance.K)]);
        if(p > maxP): maxP = p;
        if(p >= 0.5): correctCnt += c;
        else: wrongCnt += c;
    print('[Citation Prediction]:  Correct     = {0}'.format(correctCnt));
    print('[Citation Prediction]:  Wrong       = {0}'.format(wrongCnt));
    print('[Citation Prediction]:  Precison    = {0}'.format(correctCnt / (correctCnt + wrongCnt)));
    print('[Citation Prediction]:  MaxP        = {0}'.format(maxP));
    return;
...
# MAIN
#===============================================================================
def pubmedCitationTopicTimeMatrix(ldaFilePath):
...
#    ldaFilePath = os.path.join(variables.RESULT_DIR, 'pubmed_citation_lda/pubmed_citation_lda_500_145317_145317_0.001_0.001_timeCtrl_10_10.lda');
    ldaInstance = topic_modeling.lda_gibbs_sampling.readLdaEstimateFile(ldaFilePath);
    pmd = getPubMedCorpus();
    ...
    dumpFilePath = ldaFilePath + '_time';
    dumpFile = open(dumpFilePath, 'w');
    (begYr, endYr, topicTimeMatrix) = getTopicTimeDistribution(ldaInstance, pmd, pmidToId, idToPmid);
    dumpFile.write(' '.join([str(i) for i in range(begYr, endYr + 1)]) + '\n');
    dumpFile.write('\n'.join([' '.join([str(x) for x in vec]) for vec in topicTimeMatrix]));
    dumpFile.close();
...
def pubmedCitationPrediction(ldaFilePath):
    '''loading lda'''
    print('[pubmed-citation-LDA]: loading lda');
    ldaInstance = topic_modeling.lda_gibbs_sampling.readLdaEstimateFile(ldaFilePath);
    '''data loading'''
    ...
    print('                       size: {0}'.format(len(data)));
    '''running LDA'''
    getCitationPrediction(ldaInstance, data);    
    return;
